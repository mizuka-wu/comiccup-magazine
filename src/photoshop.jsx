 ...
 */
// for (var i = root.artLayers.length - rowNumber * 2 * 2; i < root.artLayers.length; i = i + colEachRow) {
//   var layer = root.artLayers[i]
//   // 每行已经有一个默认的了，所以-1
//   for (var j = 0; j < (colEachRow - 1); j++) {
//     var copy = layer.duplicate()
//     // 从后往前，位置保证一致
//     var offset = cellOffset * (colEachRow - 1 - j)
...
//   var book = left[leftIndex]
//   // 改为本子名
//   root.artLayers[artIndex].textItem.contents = book.name
//   // 改对应的社团名 因为摊位名称不是成组复制的，需要通过取余数来判断偏移(x2因为空了一行)
//   var groupIdIndex = root.artLayers.length - 1 - (leftIndex % colEachRow) - Math.floor(leftIndex / colEachRow) * colEachRow * 2
//   var stallNameIndex = groupIdIndex - colEachRow
//   root.artLayers[groupIdIndex].textItem.contents = book.groupId
//   root.artLayers[stallNameIndex].textItem.contents = book.stallName
//   // 同一个社团则隐藏名字
//   if (lastGroupId === book.groupId) {
//     root.artLayers[groupIdIndex].visible = false
//     root.artLayers[stallNameIndex].visible = false
//   }
//   // 哨兵赋值
//   lastGroupId = book.groupId
// }
...
//   var book = right[rightIndex]
//   // 本子名字填充
//   var layerSetsIndex = rowNumber - Math.floor(rightIndex / colEachRow) - 1 // 获取分组位子, 倒序
//   var artLayerIndex = colEachRow - (rightIndex % colEachRow) - 1
//   var layerSet = root.layerSets[layerSetsIndex]
//   layerSet.artLayers[artLayerIndex].textItem.contents = book.name
//   // 改对应的社团名 因为摊位名称不是成组复制的，需要通过取余数来判断偏移(x2因为空了一行), 顺便减去left所占用的格子（20 * 2）
//   var groupIdIndex = (root.artLayers.length - rowNumber * colEachRow * 2) - 1 - (rightIndex % colEachRow) - Math.floor(rightIndex / colEachRow) * colEachRow * 2
//   var stallNameIndex = groupIdIndex - colEachRow
//   root.artLayers[groupIdIndex].textItem.contents = book.groupId
//   root.artLayers[stallNameIndex].textItem.contents = book.stallName
//   // 同一个社团则隐藏名字
